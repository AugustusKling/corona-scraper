...
class Ostalbkreis extends Scraper {
    public async get() {
        const { groups } = await this.downloadAndMatch(
            ...
            /Corona-Erkrankte im Ostalbkreis<\/b><td[^>]*><b>Aus Isolation Entlassene<\/b><td[^>]*><b>Aktive Fälle<\/b><\/td><td[^>]*><b>Stand<\/b><\/td><\/tr>\s*<tr><td[^>]*>(?<cumulatedInfected>\d+)<\/td><td[^>]*>(?<cumulatedRecovered>\d+)<\/td><td[^>]*>(?<currentlyInfected>\d+)<\/td><td[^>]*>(?<updateDate>[^<]+)<\/td>/
        );
        return {
            ...groups,
            NUTS: 'DE11D',
