...
class ScraperImpl extends Scraper {
    public async get() {
        const { groups } = await this.downloadAndMatch(
            ...
            /Fälle \(Stand: (?<updateDate>.+?) Uhr\)<\/h3><ul><li>Labor-bestätigte Fälle:\s*<strong>(?<cumulatedInfected>\d+)\s*<\/strong>Personen<\/li>\s*<li>Todesfälle:\s*<strong>(?<cumulatedDeaths>\d+)\s*<\/strong>Personen<\/li>/
        );
        return {
            ...groups,
            NUTS: 'CH053',
