...
class Zollernalbkreis extends Scraper {
    public async get() {
        const matches = await this.downloadAndMatch(
            ...
            /Gesamtzahl der an dem Coronavirus-Infizierten im Zollernalbkreis: <strong>(\d+)\s*<\/strong>\(Stand: (.+) Uhr\)/
        );
        return {
            NUTS: 'DE143',
            cumulatedInfected: parseInt(matches[1], 10),
            updateDate: moment.tz(matches[2], 'D.M.YYYY, H', 'Europe/Berlin').toISOString()
        };
    }
}
